...
app.get('/api/seed', async (req, res) => {
  try {
    ...
    if (existingEvents > 0) {
      return res.json({ message: 'Already seeded', count: existingEvents });
    }

    await prisma.user.deleteMany({ where: { email: 'raph@boards.app' } });
    
    const passwordHash = await bcrypt.hash('boards2025', 10);
    const raph = await prisma.user.create({
      data: {
    ...
    res.json({ success: true, user: raph.username, event: event.title });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});
